...
encoded_name=$(curl -s "http://kids.ge/baby-name?id=$name" |grep  "<meta property=\"og:title\" content=" | grep -Po content.* | cut -d \- -f1 | cut -d \" -f2 |  perl -X -MURI::Escape -ne 'chomp;print uri_escape($_),"\n"')
